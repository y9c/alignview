...
int main(int argc, char** argv) {
  ...
  while (getline(cin, input_string)) {
    ...
    // Initialize variables for the three substring positions, the content of
    // the CIGAR and MD, and the sequences for the new read, the reference, and
    // the matches
    string read = line[9];
    int subpos = 0;
    int cigarpos = 0;
    ...
    while (subpos < line[9].size()) {
      ...
      if ((mdstream.rdbuf()->in_avail() != 0) && mdpos == 0 &&
          cigar_letter != 'S' && cigar_letter != 'I') {
        try {
    ...
    if (subpos == line[9].size()) {
      cout << line[0] << "\t" << endl
           << "Qry("
           << "+"
           << ")" << setw(10) << query_start << " " << modseq << " "
           << query_end << endl
           << left << setw(17) << " Q=" + line[4] << matches << endl
           << "Ref("
           << "+"
           << ")" << right << setw(10) << ref_start << " " << reference << " "
           << ref_end << endl
           << line[2] << "\t" << endl
           << "//" << endl;
    }
